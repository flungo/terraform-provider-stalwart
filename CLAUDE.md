...
3. **Regenerate docs.** In the web environment you cannot install Terraform locally, so push the branch — the `docs` CI workflow installs Terraform, runs `make generate`, and commits the generated output back to the branch automatically. If Terraform is available locally, run `make generate` before pushing.
